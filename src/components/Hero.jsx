...
import { heroVideo, heroPoster } from '../data/content'
...
export default function Hero() {
  ...
  // ensure the muted video autoplays once on load (and again on every refresh).
  // Set `muted` imperatively — React's JSX attr doesn't reliably set the IDL
  // property, which makes browsers block muted autoplay.
  useEffect(() => {
  ...
  return (
    <section id="top" ref={ref} className="relative flex min-h-[100svh] items-center overflow-hidden">
      {/* Background video (plays once per page load) */}
      <motion.div
      ...
      >
        <video
          ...
          playsInline
          preload="auto"
          aria-label="Тёмный шёлк — фон студии Noir Nail"
        />
      </motion.div>
